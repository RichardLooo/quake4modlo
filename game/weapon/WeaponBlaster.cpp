...
private:
	...
	stateResult_t burst(const stateParms_t& parms);
...
STATE("Burst", rvWeaponBlaster::burst)
...
stateResult_t rvWeaponBlaster::burst(const stateParms_t& parms) {
	enum {
		ALTFIRE_INIT,
		ALTFIRE_WAIT,
	};
	switch (parms.stage) {
	case ALTFIRE_INIT:
		Attack(false, 3, 10.0f, 0, 1.0f);
		PlayEffect("fx_normalflash", barrelJointView, false);
		PlayAnim(ANIMCHANNEL_ALL, "fire", parms.blendFrames);
		return SRESULT_STAGE(ALTFIRE_WAIT);
	case ALTFIRE_WAIT:
		if (AnimDone(ANIMCHANNEL_ALL, 4)) {
			SetState("Idle", 4);
			return SRESULT_DONE;
