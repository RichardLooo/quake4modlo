...
protected:
	...
	// Smoke trail: looping effect attached to the rocket while in flight.
	// Stopped (not destroyed) after smokeTrailDuration ms so emitted
	// particles can keep drifting for the remaining lifetime.
	int						smokeTrailEndTime;			// gameLocal.time when we stop emitting trail smoke

	// Explosion linger: a looping smoke/ember effect spawned at the impact
	// point.  Stopped after explosionLingerDuration ms.
	rvClientEffectPtr		explosionLingerEffect;		// handle to the linger effect
	int						explosionLingerEndTime;		// gameLocal.time when linger smoke stops
