...
idProjectile::idProjectile(void) {
	...
	smokeTrailEndTime = 0;
	explosionLingerEffect = NULL;
	explosionLingerEndTime = 0;
	bounceCount = 0;
	hitCount = 0;
	state = SPAWNED;
...
void idProjectile::Launch(const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float dmgPower) {
	...
	// Smoke trail persists for 10 seconds of travel, then emission is cut off
	// so the already-emitted particles can finish fading on their own.
	smokeTrailEndTime = gameLocal.time + SEC2MS(spawnArgs.GetFloat("smoke_trail_time", "10.0"));
...
void idProjectile::Think(void) {
	...
	if (thinkFlags & TH_PHYSICS) {
		...
		if (flyEffect) {
			...
			else if (smokeTrailEndTime > 0 && gameLocal.time >= smokeTrailEndTime) {
				// 10-second smoke trail time has elapsed — stop new emission but
				// let in-flight particles finish naturally (don't call Stop with
				// a short duration so particles already spawned can fade out).
				flyEffect->Stop(0);
				flyEffect = NULL;
				smokeTrailEndTime = 0;
			}
	...
	// Cut off the explosion linger smoke once its 20-second window is up.
	// Stop(0) ends new emission; particles already in-flight keep fading.
	if (explosionLingerEffect && explosionLingerEndTime > 0 && gameLocal.time >= explosionLingerEndTime) {
		explosionLingerEffect->Stop(0);
		explosionLingerEffect = NULL;
		explosionLingerEndTime = 0;
	}
}
...
void idProjectile::Explode(const trace_t* collision, const bool showExplodeFX, idEntity* ignore, const char* sndExplode) {
	...
	if (showExplodeFX) {
		...
		// Start a looping smoke/ember effect at the impact point.
		// It will be stopped (emission cut) after 20 seconds so any
		// in-flight particles can finish fading naturally.
		explosionLingerEffect = PlayEffect("fx_explode_linger", endpos, fxDir.ToMat3(), true);
		explosionLingerEndTime = gameLocal.time + SEC2MS(spawnArgs.GetFloat("explosion_linger_time", "20.0"));
	}
	...
	// Make sure the entity stays alive for the full explosion linger duration
	// (20 s by default) so the looping effect can keep running.
	if (explosionLingerEffect) {
		int lingerMs = SEC2MS(spawnArgs.GetFloat("explosion_linger_time", "20.0"));
		if (removeTime < lingerMs) {
			removeTime = lingerMs;
		}
		// Schedule emission cut-off: Stop(0) ends new particle emission but
		// lets already-spawned particles finish their lifetime naturally.
